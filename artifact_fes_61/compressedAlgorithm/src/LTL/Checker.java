...
public class Checker {
    ...
    static boolean[]projectedEvents;
    //current valuation.
    static Op h;
    ...
    public Checker(int buffSize, String events){
        ...
        this.projectedEvents = new boolean[150];

        for(int i = 0; i<events.length();i++){
            projectedEvents[(int) events.charAt(i)]=true;
        }
        if(events.length()==0){
            for(int i = 0;i<projectedEvents.length;i++){
                projectedEvents[i] = true;
            }
        }
        this.nonEmpty = false;
        ...
        while(!s.isEmpty()){
            ...
            if(set!=null && set.contains(buff.toString())){
                ...
                if(currentSymbol.isTerminal()){
                    ...
                    if(projectedEvents[(int)(currentSymbol.getKprefix().charAt(0))]==false){
                        continue;
                    }
                    nonEmpty = true;
        ...
        if(symbol.getKprefix()==null){
            ...
            for(int i = 0 ; i<symbol.getChildren().size();i++){
                ...
                if(currentChild.isTerminal()){
                    ...
                    if(projectedEvents[(int)e]==true&&suffix.length()< buffSize+1){
                        suffix.append(e);
                    }
                }
                else{
...

    {
        ...
        if(currentSymbol.isTerminal()){
            if(projectedEvents[currentSymbol.getKprefix().charAt(0)]==true){
